...
- [ ] **Staging behavior verified** (unless the [skip exceptions](#動作確認スキーム-必須) apply): RED reproduced + GREEN confirmed **on staging** (not just mocks), with log excerpts pasted under a `## Staging Evidence` heading (timestamp / thread / branch hash, clickable URLs). **Discord 側の証跡はユーザー提供のスクリーンショットが主**（サーバ上の AI は GUI を持たず Discord クライアントの画面そのものは撮れない）。AI は **tmux ペインキャプチャ + REST 取得メッセージ本文**で補強し、さらに **#243 の自前描画スクショ**（`/discord-screenshot` / ターン完了時の自動証跡 `<session_dir>/.evidence/*.png`、Discord 会話の忠実再現であってクライアント画面そのものではない / [docs/specs/discord-screenshot-evidence.md](docs/specs/discord-screenshot-evidence.md)）も証跡にできる。長いログは `<details>` で畳む。**An empty Staging Evidence section = not done.**
