"""Tests for the c-lord CLI wizard.
...
class TestValidateToken:
    ...
    async def test_returns_bot_name_on_success(self) -> None:
        from c_lord.cli import validate_token
...
class TestFetchChannels:
    ...
    async def test_returns_text_channels(self) -> None:
        from c_lord.cli import fetch_channels
...
class TestWriteEnv:
    ...
    def test_writes_required_vars(self, tmp_path: Path) -> None:
        from c_lord.cli import write_env
...
class TestCheckClaudeCli:
    ...
    def test_returns_true_when_installed(self) -> None:
        from c_lord.cli import check_claude_cli
...
class TestMain:
    ...
    def test_no_args_shows_help(self, capsys: pytest.CaptureFixture[str]) -> None:
        from c_lord.cli import main

        with patch("sys.argv", ["c-lord"]), pytest.raises(SystemExit) as exc_info:
            main()
    ...
    def test_unknown_command_exits_nonzero(self) -> None:
        ...
        with patch("sys.argv", ["c-lord", "nonexistent"]), pytest.raises(SystemExit) as exc_info:
            main()
        assert exc_info.value.code != 0
    ...
    def test_start_without_env_exits_with_message(
        ...
        from c_lord.cli import cmd_start
