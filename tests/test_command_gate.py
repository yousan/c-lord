...
class TestOwns:
    ...
    async def test_home_channel_is_ours(self) -> None:
        assert await owns(_bot(), channel_id=HOME, thread_id=None) is True

    @pytest.mark.asyncio
    async def test_thread_under_home_channel_is_ours(self) -> None:
        assert await owns(_bot(), channel_id=HOME, thread_id=999) is True
    ...
    async def test_foreign_channel_without_any_trace_is_not_ours(self) -> None:
        ...
        assert await owns(_bot(), channel_id=FOREIGN, thread_id=999) is False

    @pytest.mark.asyncio
    async def test_channel_binding_makes_it_ours(self) -> None:
        bot = _bot()
        bot.get_cog.return_value = _channel_cog(tmux=object())
        assert await owns(bot, channel_id=FOREIGN, thread_id=None) is True

    @pytest.mark.asyncio
    async def test_session_dir_binding_makes_it_ours(self) -> None:
        bot = _bot()
        bot.get_cog.return_value = _channel_cog(sdm=object())
        assert await owns(bot, channel_id=FOREIGN, thread_id=None) is True
    ...
    async def test_session_row_makes_the_thread_ours(self) -> None:
        ...
        assert await owns(bot, channel_id=FOREIGN, thread_id=999) is True
    ...
    async def test_session_row_is_only_consulted_for_threads(self) -> None:
        ...
        bot.session_repo = MagicMock()
        bot.session_repo.get = AsyncMock(return_value={"thread_id": 999})
        assert await owns(bot, channel_id=FOREIGN, thread_id=None) is False
        bot.session_repo.get.assert_not_called()
    ...
    async def test_a_failing_lookup_does_not_claim_the_channel(self) -> None:
        ...
        bot = _bot()
        cog = _channel_cog()
        cog.resolve_tmux_manager = AsyncMock(side_effect=RuntimeError("db down"))
        cog.resolve_manager = AsyncMock(side_effect=RuntimeError("db down"))
        bot.get_cog.return_value = cog
        assert await owns(bot, channel_id=FOREIGN, thread_id=None) is False


class TestOwnsChannel:
    """The object-shaped entry point used by ``process_commands``."""
