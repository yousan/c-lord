...
**判定は「怪しければ落とす」側に倒してある。** 例えば撮影アカウントに
**送信権限が無いチャンネル**では、入力欄の代わりに「メッセージを送信する権限が
ありません」のバーが出るので `not-a-channel` に触れる可能性がある。
黙って無価値な PNG を通すより、理由つきで落として人が見るほうが安全という
判断。誤検出だと確認できたときだけ `--no-qc` を使う。
