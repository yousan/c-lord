...
- 通知先は**そのターンを投げた人**（初回プロンプトならスレッド作成者、リプライならリプライした人）。
- Webhook / スケジュール実行など人間の投稿者がいないターンや、tmux ペインで直接操作したターンでは、`DISCORD_OWNER_ID` が設定されていればそのオーナーに通知されます（未設定なら通知しません）。
