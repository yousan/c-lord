...
**選択肢（質問やプラン承認のボタン）への回答を待っているあいだも表示されません。**
ペインは誰かが答えるまで動かないのが正しい姿で、何分置いてもハングではないからです。
5 分を過ぎてもボタンで答えられ、答えればそのまま続きが返ります（#751）。
一方、Claude の作業中スピナー（`✻ … (12s · …)`）が出たままペインが止まった場合は
引き続き通知します。スピナーのタイマーは正常なら毎秒描き直されるので、止まっている
のは Claude が描画を止めた＝本当に固まったときだからです。
