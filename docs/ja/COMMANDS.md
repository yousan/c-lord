...
| コマンド | 説明 | 使用場所 |
|---------|------|---------|
| `/thread-rename` | このスレッドの名前を、直近の会話から要約し直す | スレッドのみ |

**c-lord はスレッド名を自分から要約しません（#705）。** 新しいスレッドは開いたときの名前
（あなたが付けた名前、または最初のメッセージ）をそのまま持ち続け、その周りに `W<番号> │` と
`#<Issue番号>` が付くだけです。サイドバーは自分のスレッドを見分けるための場所なので、
頼んでいないのに名前が変わると、そのスレッドを見失うからです。

`/thread-rename` はその裏返しで、名前が古びたときに**自分で**呼ぶコマンドです。直近の会話を
**sonnet** で 20 字以内に要約してトピックだけ差し替えます。`W<番号> │`・別リポジトリのときの
`<リポジトリ名>:W<番号> │`（#618）・`#<当初の番号> … →#<いまの番号>`（#593）・`[停止]`（#512）は
そのまま残ります。手で付けた名前（#95 のロック済み）でも効きます — ロックが止めるのは
*c-lord が勝手に* 変えることで、このコマンドはあなたが頼んでいるからです。
旧挙動（初回メッセージを LLM で要約）に戻すには `CLORD_AUTO_TOPIC=1`。
詳細は [specs/thread-name.md](../specs/thread-name.md)。
...
| `!thread-rename` | このスレッドの名前を要約し直す（sonnet） | `!thread-rename` |
...
どちらも未設定の場合は、c-lord が Discord に**アプリの所有者**を問い合わせ、その
アカウント（チーム所有ならそのメンバー）だけを許可します。設定は不要で、かつサーバに
開放もされません。**c-lord に話しかけられる＝そのホストでシェルを実行できる**ので、
「未設定」が「全員許可」であってはならないためです (#713)。誰が許可されたかは起動ログに出ます。

3. **明示的に全員許可** — `CLORD_ALLOW_ANYONE=1` を設定すると #713 以前の挙動（サーバの
   全メンバーが Bot を操作できる）に戻せます。設定されている場合、起動時に警告が出ます。

詳細は [../specs/authorization-default.md](../specs/authorization-default.md)。
