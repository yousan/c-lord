...
각 Discord 스레드는 격리된 Claude Code 세션이 됩니다. 필요한 만큼 세션을 실행하세요: 한 스레드에서 기능을 개발하고, 다른 스레드에서 PR을 검토하고, 세 번째 스레드에서 예약된 작업을 실행합니다. 각 스레드는 저장소의 자체 클론에서 작업하므로, 세션들이 같은 작업 트리를 편집하는 일은 없습니다.
...
## 핵심 아이디어: 스레드마다 자체 클론을 가진 병렬 세션

각 Discord 스레드는 서버의 자체 tmux 창에서 실행되는 독립된 Claude Code 세션입니다. 여러 스레드에 작업을 보내면 나란히 실행되고, 노트북을 닫아도 계속 실행되며, 스레드에 답장하면 중단된 곳에서 이어집니다.

병렬 세션이 서로의 작업을 망가뜨리지 않게 하는 것은 **격리**입니다: 채널이 저장소에 바인딩되어 있으면(`/clord-init`) 모든 스레드가 각자 독립된 `git clone` 에서 작업합니다. 세션은 작업 트리를 절대 공유하지 않으므로, 한 스레드의 커밋되지 않은 변경이 다른 스레드에 나타날 수 없습니다. 세션이 만나는 곳은 git 리모트(브랜치, push, PR)뿐이며, 사람 기여자들과 같은 방식입니다.

```
스레드 A (기능)    ──→  Claude Code  (자체 클론)  ─┐
스레드 B (PR 검토) ──→  Claude Code  (자체 클론)   ├─→  git 리모트 (브랜치 · PR)
스레드 C (문서)    ──→  Claude Code  (자체 클론)  ─┘
```

**현재 세션끼리는 서로 조율하지 않습니다.** 이 README의 이전 버전은 네 가지 조율 메커니즘을 설명했지만, 실제 상태는 다음과 같습니다 ([#758](https://github.com/yousan/c-lord/issues/758)):

| 메커니즘 | 실제 상태 |
|---|---|
| **동시성 알림** (병렬 작업에 관한 Claude용 지침) | 매 턴 생성되지만 **전달되지 않음** — 세션은 tmux TUI에서 실행되며 (#53), 여기에는 턴마다 시스템 프롬프트를 넘길 경로가 없어 텍스트가 버려짐 |
| **활성 세션 레지스트리** | 메모리에 존재하며 `/workspace-cleanup` 이 사용하지만, **세션 자신은 이를 보지 못함** (같은 버려지는 텍스트에 실려 있음) |
| **조율 채널** | **기본값은 꺼짐.** `COORDINATION_CHANNEL_ID` 를 설정하면 세션의 턴이 끝날 때 한 줄 알림을 게시함; 시작 이벤트는 없음 |
| **AI Lounge** | REST 엔드포인트는 동작하지만 **Claude에게 라운지의 존재를 알려주는 것이 없어**, 실제로는 아무것도 게시되지 않음 — [#758](https://github.com/yousan/c-lord/issues/758) 참조 |

즉, 실시간 조율이 아니라 분리된 작업 트리입니다. 클론 바깥의 것 — 같은 리모트 브랜치, 공유 포트나 데이터베이스, staging 봇 — 은 여전히 충돌할 수 있습니다. 스레드마다 자체 브랜치를 사용하세요.
...
여러 스레드를 동시에 엽니다. 각각은 자체 컨텍스트와 저장소의 자체 git 클론을 가진 독립적인 Claude Code 세션입니다. 유용한 패턴:

- **기능 + 검토 병렬**: 한 스레드에서 기능을 시작하는 동안 Claude가 다른 스레드에서 PR을 검토합니다.
- **여러 기여자**: 다른 팀원들이 각자 스레드와 자체 클론을 가지므로, 세션이 디스크상의 같은 파일을 편집하는 일은 없습니다.
...
### 동시성 & 조율
- **스레드마다 git 클론 하나** — 채널이 저장소에 바인딩되어 있으면(`/clord-init`) 각 스레드가 자체 독립 `git clone` 에서 작업; 세션끼리 공유하는 것은 git 리모트뿐
- **활성 세션 레지스트리** — 실행 중인 세션의 인메모리 목록으로, `/workspace-cleanup` 이 사용. 세션 자신은 서로에 대해 알지 못함 (#758)
- **AI Lounge 엔드포인트** — `GET/POST /api/lounge` 가 짧은 메모를 저장하고 Discord 채널로 전달. **세션에 주입되지 않음** (#758): 직접 지시한 경우에만 Claude가 사용
- **조율 채널** — 선택 사항 (`COORDINATION_CHANNEL_ID`, 기본값은 꺼짐): 세션의 턴이 끝날 때 한 줄 알림을 게시
...
- **워크스페이스 정리** — `/workspace-cleanup` 이 실행 중인 세션이 사용하지 않는 클론 디렉토리를 회수 (`dry_run` 으로 미리보기); 커밋되지 않은 변경이 있는 디렉토리는 절대 제거되지 않음
...
| `COORDINATION_CHANNEL_ID` | 세션의 턴이 끝날 때 한 줄 알림을 받는 채널; AI Lounge의 기본 채널이기도 함 | (선택) |
